...
def organizaArquivosdeSmap(cxv, nDias, nome):
    ...
    for x in cxv.regioes:
        ...
        lista = caso(i)
        postos(lista, data, i, x, nome, path_to_model)
...
def postos(listaa, data, path, regiao, nome, path_to_model):
    ...
    for i in listaa:
        ...
        if i.upper() + '.txt' in os.listdir(path):
            ...
            nova_linha = nova_linha.replace(nova_linha.split('|')[5], vazaoObservada(i, data_rodada, regiao, nome, path_to_model))
...
def vazaoObservada(sub_bacia_modelada, data_procurada, regiao_smap, nome_previsão, path_to_model):

    file = path_to_model + '\\' + regiao_smap + '\\ARQ_SAIDA' + '\\' + sub_bacia_modelada + '_' + nome_previsão + '_PREVISAO.txt'

    with open(file, 'r') as saida_smap:

        txt = saida_smap.readlines()

    aux = [i.split() for i in txt]
    
    for i in aux:
        print('i :', i, '\n')
        if i[0] == data_procurada.strftime('%d/%m/%Y'): return i[-1]
    raise Exception('Algum erro com relação a leitura do documento de saida do SMAP, para a realização do preliminar')
